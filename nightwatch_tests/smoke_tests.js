...
const hello_how_are_you_feelin_text_cssSelector = "#root > div > div > div > h2";
...
const select_from_the_options_below_text_cssSelector = "#root > div > div > div > p";
...
module.exports = {
  ...
  "2. Hello How are you feeling today": async function (browser) {
    ...
    await browser.assert.containsText(hello_how_are_you_feelin_text_cssSelector, "feeling");
  },

  "3. Please select from the images below": async function (browser) {
    browser.url("https://project32.s3.eu-west-2.amazonaws.com/index.html");

    await browser.expect.element(select_from_images_text_cssSelector).to.be.visible;
  },
  ...
  "6. Please select from the options below is not there": async function (browser) {
    browser.url("https://project32.s3.eu-west-2.amazonaws.com/index.html");

    const cssSelector =
      "#root > div > div > div > div > button.EmojiButton_EmojiButton__1_DRA.EmojiButton_EmojiButton_selected__2Nkuv > span";
    await browser.click("#root > div > div > div > div > button:nth-child(1) > span");
    await browser.expect.element(select_from_the_options_below_text_cssSelector).to.be.not.present;
  },
